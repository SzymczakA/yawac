...
net_change() {
        ...
        while [ "$scanres" = "" ]; do
                ...
                scanres=$(iw scan1 scan | grep SSID)
        done
...
freewifi_do_connect() {
        ...
        while [ "1" ]; do
                ...
                if [ $? -eq 0 ] && [ ${WGET_OUT} -eq 0 ]; then
                        # echo "Modification du MTU de l'interface wlan1..."
                        # ifconfig wlan1 mtu 1460 2>/dev/null
                        # echo "Le MTU a ete modifie : $(ifconfig wlan1 | sed -n 's/.*mtu \([0-9]*\).*$/\1/p')"
...
rand_mac_addr() {
        ...
        # uci set wireless.radio1.macaddr="$macaddr"
        uci set wireless.@wifi-iface[1].macaddr="$macaddr"
        uci commit wireless
        /etc/init.d/network restart
}
